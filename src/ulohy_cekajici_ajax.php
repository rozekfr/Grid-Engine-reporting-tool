...
<?php
    ...
    //parsování čekajících úloh
    include "pending_job_parser.php";
    ...
    //dotaz do DB
    $dotazJobs = $db->query("SELECT $select FROM `cekajici_ulohy` WHERE $where ORDER BY $orderby");

    //nastavení statistik - filtrů a stránkování
    echo "<div id='stats_settings'>";
    echo "<span>Počet položek: </span>";
    echo "<select id='pocet_polozek' onchange='update_stats(null,1,\"ulohy_cekajici_ajax\",\"jobs_stats\")'>";
    ...
    echo "<select id='strankovani' onchange='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\")'>";
    ...
    echo "<input type='button' id='filtr' value='aplikovat filtry' onclick='update_stats(null,1,\"ulohy_cekajici_ajax\",\"jobs_stats\")'>";
    echo "</div>";
    ...
    if(!isset($select)){
        ...
        foreach($columns as $column){
            if(!empty($column)){
                switch($column){
                    case 'id_ulohy':
                        echo "<th>ID úlohy <img src='foto/".($order == $column ? ($howorder == "ASC" ? "vzestupne" : "sestupne") : "neradit").".png' id='id_ulohy' class='razeni' alt='razeni' onclick='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\");'></th>";
                        break;

                    case 'uzivatel':
                        echo "<th>Uživatel <img src='foto/".($order == $column ? ($howorder == "ASC" ? "vzestupne" : "sestupne") : "neradit").".png' id='uzivatel' class='razeni' alt='razeni' onclick='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\");'></th>";
                        break;

                    case 'pocet_tasku':
                        echo "<th>Počet tasků <img src='foto/".($order == $column ? ($howorder == "ASC" ? "vzestupne" : "sestupne") : "neradit").".png' id='$column' class='razeni' alt='razeni' onclick='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\");'></th>";
                        break;

                    case 'cas_odeslani':
                        echo "<th>Čas odeslání <img src='foto/".($order == $column ? ($howorder == "ASC" ? "vzestupne" : "sestupne") : "neradit").".png' id='$column' class='razeni' alt='razeni' onclick='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\");'></th>";
                        break;

                    default:
                        echo "<th>".ucfirst($column)." <img src='foto/".($order == $column ? ($howorder == "ASC" ? "vzestupne" : "sestupne") : "neradit").".png' id='$column' class='razeni' alt='razeni' onclick='update_stats(this,1,\"ulohy_cekajici_ajax\",\"jobs_stats\");'></th>";
                        break;
                }
            }
        }
        echo "<th>Další informace</th>";
        echo "</tr>";
    }
    echo "</thead>";
    ...
    echo "<td class='filtr'></td>";
    echo "</tr>";
    //data
    $dotazJobs = $db->query("SELECT $select FROM `cekajici_ulohy` WHERE $where ORDER BY $orderby $limit");
    if(is_object($dotazJobs) and $dotazJobs->num_rows != 0){
        while($job = $dotazJobs->fetch_assoc()){
            ...
            echo "<td><input type='button' value='zjistit' onclick='get_info_request({$job["id_ulohy"]})'></td>";
            echo "</tr>";
        }
    }
    else{
